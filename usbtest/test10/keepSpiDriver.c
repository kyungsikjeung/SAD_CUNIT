...
/* 제어기 Deserializer SPI 데이터 전송 전 초기화 */
void SPI_BypassDriver_Init(){
    PRINTF("SPI_BypassDriver_Init\r\n");
    GPIO_OutputBypass(SPI_EIRQ, 1u);
	registerBypassSetting(); 
    TIMER_DelayMsec(10);
    // !Ramres to RO(OUTPUT) 
    DisableIRQ(PORTC_IRQn); // SPI 전송 시 RO 사용하므로, Ramres 인터럽트 발생되지 않도록 설정
    gpio_pin_config_t outputConfig = {
		kGPIO_DigitalOutput, 1,
	};
    // RO Input 으로 설정 - Serializer에서 데이터를 제어할 때 사용됨 - 데이터 시트 반드시 확인할것, RO1 명령전송 모드 RO0 데이터전송모드
    ...
    // ! Bypass Path 설정 (만약 Indigo에서 제어를 할 경우, GPIO 11을 LOW로 설정)
   // SPI_BypassDriver_SetPath(CONNECT_FLASH_REGISTER_VALUE); // External Flash Memory 연결
}

/* 제어기 Deserializer SPI 데이터 전송 후 */
void SPI_BypassDriver_DeInit(){
    PRINTF("SPI_BypassDriver_DEInit\r\n");
    GPIO_InputPin_Init(); //Ramres,Sys,HostInterrupt 설정
    /* BNE CS */
    PORT_SetPinMux(BNE_PORT, BNE_PIN, kPORT_MuxAlt2);
    
    GPIO_OutputBypass(SPI_EIRQ, 0u); 
}

// /* Deserializer Bypass Path 설정 -  GPIO11의 OUTPUT HIGH일 경우 FLASH, 0일경우 인디고 */
 void SPI_BypassDriver_SetPath(BypassPath path){
 	i2c_register_t GPIO11_OutputValue = {0x48,0x0221,(uint8_t)path,0}; // HIGH
 	uint8_t slaveRegisterValue = GPIO11_OutputValue.registerValue;
 	I2C_Write(I2C1,GPIO11_OutputValue.slaveAddr,GPIO11_OutputValue.registerAddr, 2, &slaveRegisterValue , 1);
 }
